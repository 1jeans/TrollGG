...
<div style="margin: 3% 15% 0 15%;">
    ...
    <div align="center" style="display: flex; justify-content: center;">
        <div align="justify"
             style="border: 5px solid #BDBBBB; width: 50%; height: 170px; background-color: white; display: flex; flex-direction: column;">
            <div align="center"
            ...
            <div style="display: flex;">
                ...
                <div style="display: flex;">
                    ...
                    <div style="margin-top: 15px; display: flex; flex-direction: column;">
                        <a id="link1">
                            <div id="gamername1"
                                 style="font-size: 20px; color: black; display: flex; flex-direction: column;">

                    </div></a>
                    <div id="rank"
                         style="display: flex; font-size: 12px; margin: 5px 0 5px 20px; display: flex; align-items: center;">
                        <img src="/resource/res/img/emblems/Emblem_CHALLENGER.png" style="height: 30px;"/>
                        Challenger
                        <div id="rank_point1">
                            <b>point</b>
                        </div>
            ...
            <div style="margin-left: 10px; display: flex; flex-direction: column; margin: 0 5% 0 10%;">
                <c:forEach var="a" begin="0" end="2" step="1">
                    <div style="display: flex; align-items: center; margin-bottom: 10px;">
                        ...
                        <!-- 모스트1 챔피언 이름 -->
                        <div class="mostChampion" id="championName${a}" style="font-size: 12px; color: red;">챔피언
                        </div>
                        <!-- 모스트1 챔피언 레벨-->
...
<div style="display: flex; justify-content: space-between;">
    <c:forEach var="b" begin="2" end="5" step="1">
        <div align="justify"
             style="border: 5px solid #BDBBBB; width: 250px; height: 170px; background-color: white;">
            <div align="center"
                 style="background-color: #BDBBBB; color: white; font-size: 20px; padding: 3px; width: 20px;">
                <b>${b}</b>
            </div>
            <div align="center">
                ...
                <div id="rank_stat${b}" style="display: flex; justify-content: center;">
                    <div style="display: flex; width: 130px; border-radius: 10px; margin-right: 10px;">
                        <div align="left" id="win_bar${b}"
                             style="background-color: #3D95E5; width: 62%; color: white; border-radius:">134
                        </div>
                        <div align="right" id="lose_bar${b}"
                             style="background-color: #EE5A52; width: 38%; color: white;">83
                        </div>
                    </div>
...
<div style="border: 0.3px solid gray; width: 100%; height: 2000px; background-color: #F2F2F2;">
    ...
    <!-- 1칸 -->
    <c:forEach var="g" begin="6" end="40" step="1">
        <div style="display: flex; background-color: #EAEAEA; height: 50px;">
            <div align="center" style="width: 10%; display: flex; align-items: center;">${g}</div>
            <div style="width: 30%; display: flex; align-items: center;">
                <img class="circle_image" id="pfimg_rank${g}" src=""
                     style="width: 30px; height: 30px; margin-right: 5px; border-radius: 15px;"/>
                <a id="link${g}">
                    <div id="gamername${g}">보고싶다 오로라</div>
                </a>
            </div>
            <div style="width: 10%; display: flex; align-items: center;">Challenger</div>
            <div id="rank_point${g}" style="width: 13%; display: flex; align-items: center;">1,372 LP</div>
            <div id="gamerLV${g-1}" style="width: 13%; display: flex; align-items: center;">225</div>
            <div style="width: 24%; display: flex; align-items: center;">
                <div style="display: flex; width: 130px; border-radius: 10px; margin-right: 10px;">
                    <div align="left" id="win_bar${g}"
                         style="background-color: #3D95E5; width: 62%; color: white; border-radius:">134
                    </div>
                    <div align="right" id="lose_bar${g}"
                         style="background-color: #EE5A52; width: 38%; color: white;">83
                    </div>
                </div>
                <div id="winper${g}">62%</div>
            </div>
        </div>
    </c:forEach>
</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            rankingNum = res.entries;
            rankingNum.sort(function (c, d) {
                return d.leaguePoints - c.leaguePoints;
            })
            for (var g = 6; g < 41; g++) {
                document.querySelector("#link" + g).href = "http://localhost:8091/search?SummonerName=" + rankingNum[g - 1].summonerName;
                document.querySelector("#gamername" + g).innerHTML = "<b>" + rankingNum[g - 1].summonerName + "</b>";
                document.querySelector("#rank_point" + g).innerHTML = "<b>" + rankingNum[g - 1].leaguePoints + " LP</b>";
                document.querySelector("#win_bar" + g).innerHTML = rankingNum[g - 1].wins;
                document.querySelector("#lose_bar" + g).innerHTML = rankingNum[g - 1].losses;
                document.querySelector("#winper" + g).innerHTML = Math.round(rankingNum[g - 1].wins / (rankingNum[g - 1].losses + rankingNum[g - 1].wins) * 100) + "%";
                $.ajax({
                    type: "GET",
                    url: "https://kr.api.riotgames.com/lol/summoner/v4/summoners/by-name/" + rankingNum[g - 1].summonerName + "?api_key=" + api_key,
                    dataType: "json",
                    async: false
                }).done((res) => {
                    if (g == 1) {
                    ...
                    console.log(res);
                    document.querySelector("#gamerLV" + (g - 1)).innerHTML = "<b>" + res.summonerLevel + " LV</b>";
                    document.querySelector("#pfimg_rank" + (g)).src = "http://ddragon.leagueoflegends.com/cdn/${summonerVersion}/img/profileicon/" + res.profileIconId + ".png";
                })
            }
        });
    });
